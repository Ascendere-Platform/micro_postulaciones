...
func EliminoMiembro(id string, miembro apimodels.UsuarioEquipo) (string, error) {
	...
	for i, persona := range resultado.Equipo {
	...
	if pos == 0{
		return "No se encuentra el miembro del equipo", err
	}

	resultado.Equipo = append(resultado.Equipo[:pos], resultado.Equipo[pos+1:]...)
	...
	if errUpdt != nil {
		return "No se puede actualizar", err
	}
